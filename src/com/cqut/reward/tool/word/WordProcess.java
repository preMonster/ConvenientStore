package com.cqut.reward.tool.word;
...
public class WordProcess {  
     ...
     *            行数 
     */  
    public void createTable(String pos, int numCols, int numRows,List<Map<String, Object>> list) {  
        if (find(pos)) {  
    		...
    		for(int i=1;  i<=list.size(); i++)
    			for(int j=1; j<=list.get(0).size(); j++){
    				  Dispatch cell = Dispatch.call(newTable, "Cell", new Variant(i),  
